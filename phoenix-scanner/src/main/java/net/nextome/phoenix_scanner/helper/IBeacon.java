...
public class IBeacon {
    ...
    public static IBeacon fromScanData(byte[] scanData, int rssi) {
        ...
        iBeacon.rssi = rssi;
        int byte32 = scanData[32] & 0xFF;
        int byte33 = scanData[33] & 0xFF;
        byte[] batt = new byte[2];
        batt[0] = (byte) byte32;
        batt[1] = (byte) byte33;
        String producer = bytesToHex(batt);

        if(producer.equalsIgnoreCase("beac") || producer.equalsIgnoreCase("acbe")){
            //BlueUp Beacons old firmware


            iBeacon.battery = scanData[60];
        }else{
            batt = new byte[]{ scanData[5], scanData[6],scanData[7],scanData[8]};
            producer = bytesToHex(batt);
            //Kontakt.io producer
            if(producer.equalsIgnoreCase("4c000215")){
                //Kontakt battery
                iBeacon.battery = scanData[52];
            }
        }
    ...
    public IBeacon(String proximityUuid, int major, int minor, int txPower, double rssi) {
